...
{
	$id=$_GET["id"];
	$str_house="SELECT * FROM tb_house WHERE office_none not in(2) and id_product=".$id;
	$data_house=$lib->selectall($str_house,$product_max);
	$sql_room="select * from tb_house,tb_rooms where tb_house.id_house=tb_rooms.id_house and tb_house.id_product=".$id;
	$data_room=$lib->selectall($sql_room);
	$link=$url.'product/'.$id.'/p';
...
//	$sql_room_type="SELECT *,(quantity-room_rent) as room_empty FROM tb_project,tb_house,tb_rooms 
//				where tb_project.id=tb_house.id_project 
